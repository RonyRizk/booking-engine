  ...
  return (
    <Row key={room.id} style={{ marginBottom: "15px", width: "100%" }}>
      <Column align={lang === "ar" ? "right" : 'left'} style={{ width: "70%", verticalAlign: 'top', }}>
        <EmailText style={{ margin: 0 }}><b>{room.roomtype.name} - {room.rateplan.short_name} {custom_text}</b> <br /> {room.guest.first_name} - <span dangerouslySetInnerHTML={{ __html: printingService.formatGuestAvailability(room.occupancy, locales) }}></span>.
